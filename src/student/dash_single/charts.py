...
path_sc = pathlib.Path(__file__).parent.parent.joinpath("data", "paralympics.db")
...
def scatter_geo():
    with path_sc:
        ...
        connection = sqlite3.connect(path_sc)
...
def create_card(host_year):
    ...
    Parameters:
        host_year: str String with the host city name followed by a spacde and then the year

    Retrurns:
        card: dash boostrap component card for the event
    '''
    ...
    with path_sc:
        ...
        with conn:
            ...
            highlights = event_df['highlights'].values[0]
            participants = event_df['participants'].values[0]
            events = event_df['events'].values[0]
            countries = event_df['countries'].values[0]
            ...
            card = dbc.Card([
                ...
                dbc.CardBody([
                    html.H4(host_year, 
                            className=host_year),
                    html.P(f'Highlights: {highlights}', className="Highlights", ),
                    html.P(f'Participants: {participants}', className="Participants", ),
                    html.P(f'Events: {events}', className="Events", ),
                    html.P(f'Countries: {countries}', className="Countries", ),
                ]),
            ],
                style={"width": "18rem"},
            )
